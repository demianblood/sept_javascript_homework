...
//     div.innerText = `${item.name} ${item.surname} ${item.age}років   ${item.info}`;
...
coursesArray.forEach(item => {
    ...
    div.classList.add('complex-info');
    divInfo.classList.add('time-info')
    divModules.classList.add('module-list')

    for (let itemKey in item) {
        if (itemKey === 'title') {
            let h1 = document.createElement('h1');
            h1.innerText = item[itemKey]
            h1.classList.add('h1')
            div.appendChild(h1)
        } else if (itemKey === 'monthDuration' || itemKey === 'hourDuration') {
            let divDuration = document.createElement('div');
            divDuration.innerText = item[itemKey];
            divInfo.appendChild(divDuration);
            div.appendChild(divInfo);
        } else if (itemKey === 'modules'){
            item[itemKey].forEach(module =>{
